...
public class InstructionObject {
	...
	public InstructionObject(String instr) {
            ...
            } else if (!linea[0].equals("READ") && !linea[0].equals("WRITE")&& !linea[0].equals("CREATE") && !linea[0].equals("DESTROY") && !linea[0].equals("RUN")) {
		this.instruccion = "BAD";
           
            // Reviso si la instruccion es del largo apropiado acorde a la operacion a ejecutar
            }else if ((linea.length != 4 && linea[0].toUpperCase().equals("WRITE")) || (linea.length != 3 && linea[0].toUpperCase().equals("READ")) || (linea.length != 3 && linea[0].equals("CREATE"))
                || (linea.length != 3 && linea[0].equals("DESTROY")) || (linea.length != 2 && linea[0].equals("RUN"))) {
                this.instruccion = "BAD";
            ...
            } else if (linea[0].equals("DESTROY") && !ObjectManager.obtenerObjetos().containsKey(linea[2])) {
                   this.instruccion = "BAD";
                   
            //existe el objeto a crear?
            } else if (linea[0].equals("CREATE") && ObjectManager.obtenerObjetos().containsKey(linea[2])) {
                this.instruccion = "BAD";
            }
             
            if(this.instruccion != "BAD"){
                if (linea[0].toUpperCase().equals("READ")) {
                    this.instruccion = "READ";
                    this.sujeto = linea[1];
                    this.objeto = linea[2];
                    ReferenceMonitor.Leer(obtenerInstructionObject());
                } else if (linea[0].toUpperCase().equals("WRITE")) {
                    this.instruccion = "WRITE";
                    this.sujeto = linea[1];
                    this.objeto = linea[2];
                    this.valor = Integer.parseInt(linea[3]);
                    ReferenceMonitor.Escribir(obtenerInstructionObject());
                } else if (linea[0].equals("CREATE")) {
                    this.instruccion = "CREATE";
                    this.sujeto = linea[1];
                    this.objeto = linea[2];
                    ReferenceMonitor.Crear(obtenerInstructionObject());
		} else if (linea[0].toUpperCase().equals("DESTROY")) {
